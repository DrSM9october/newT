    ...
    <header className="sticky top-0 z-40 bg-white border-b border-slate-200/90 px-3 md:px-6 py-2.5 shadow-xs">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-2.5">
        ...
        <div className="flex items-center justify-between w-full md:w-auto gap-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500 font-medium">دستیار تخصصی زبان و لهجه‌ها</p>
            </div>
          </div>

          {/* Accent Quick Selector on Mobile */}
          <div className="md:hidden flex items-center gap-1 bg-slate-100 px-2 py-1 rounded-xl border border-slate-200 text-xs">
        ...
        <div className="hidden md:flex items-center gap-2 bg-slate-100 px-3 py-1.5 rounded-xl border border-slate-200 text-xs">
          ...
          <span className="text-slate-500 font-medium text-[11px]">لهجه/زبان:</span>
          <select
            value={activeDialect}
            onChange={(e) => setActiveDialect(e.target.value as DialectType)}
            className="bg-transparent text-slate-800 font-bold text-xs focus:outline-none cursor-pointer"
          >
            ...
            <option value="ar-IQ">🇮🇶 عربی (لهجه عراقی)</option>
            <option value="ar-LB">🇱🇧 عربی (لهجه لبنانی)</option>
          </select>
        </div>
      </div>
    </header>
