...
const mainActivityRoot = path.join(root, 'android', 'app', 'src', 'main', 'java');
...
const mainActivity = findFile(mainActivityRoot, 'MainActivity.java');
...
if (!mainActivity) throw new Error('Generated Capacitor MainActivity.java not found.');

const source = fs.readFileSync(mainActivity, 'utf8');
const packageMatch = source.match(/^package\s+([\w.]+);/m);
if (!packageMatch) throw new Error('Unable to determine Android package from MainActivity.java.');
const pkg = packageMatch[1];

const nativeTemplatePath = path.join(root, 'android-native-template', 'LinguaVoicePackManager.java');
const packagePath = path.join(mainActivityRoot, ...pkg.split('.'));
fs.mkdirSync(packagePath, { recursive: true });
if (!fs.existsSync(nativeTemplatePath)) throw new Error('LinguaVoicePackManager template not found.');
const managerSource = fs.readFileSync(nativeTemplatePath, 'utf8').replace('__PACKAGE__', pkg);
fs.writeFileSync(path.join(packagePath, 'LinguaVoicePackManager.java'), managerSource);
...
console.log(`Patched native Android TTS bridge: ${mainActivity}`);